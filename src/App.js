...
class App extends Component {
  ...
  deleteTrack = id => {
    ...
    const updatedTrackList = trackList.filter(eachTract => eachTract.id !== id)
    this.setState({
  ...
  render() {
    ...
    const filteredTrackList = trackList.filter(eachTract =>
      eachTract.name.toLowerCase().includes(searchInput.toLowerCase()),
    )
        ...
        <div className="songs-playlist-container">
          <div className="heading-and-input-container">
            ...
            <div className="input-container">
              ...
              />
              <BsSearch color="#cbd5e1" size={20} />
            </div>
          </div>

          {filteredTrackList.length === 0 ? (
            <div className="no-song-found-container">
              <p className="heading">No Songs Found</p>
            </div>
          ) : (
            <ul className="playlist-container">
              {filteredTrackList.map(eachTract => (
                <ListItem
                  key={eachTract.id}
                  eachTract={eachTract}
